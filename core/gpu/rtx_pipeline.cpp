...
rtx_trace_impl(const RtxCtx& ctx, const RtAccel& accel,
    ...
    auto rb = mk_buf(ctx, img_bytes, VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                     VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                     VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, false);
    ...
    {
        void* p = nullptr;
        vkMapMemory(dev, rb->mem, 0, img_bytes, 0, &p);
    ...
    vkDestroyBuffer(dev, rb->buf, nullptr); vkFreeMemory(dev, rb->mem, nullptr);
...
rtx_trace_groups(const RtxCtx& ctx, const RtAccel& accel,
    ...
    VkShaderModule m_rgen  = mk_module(dev, rgen);
    VkShaderModule m_rmiss = mk_module(dev, rmiss);
    VkShaderModule m_rchit = mk_module(dev, rchit);
    std::vector<VkShaderModule> m_rint(ng);
    ...
    std::uint32_t hsize  = rtprops.shaderGroupHandleSize;
    std::uint32_t balign = rtprops.shaderGroupBaseAlignment;
    std::uint32_t halign = rtprops.shaderGroupHandleAlignment;
    VkDeviceSize hstride = align_up(hsize, halign);
    ...
    VkDeviceSize rgen_sz = align_up(hstride, balign);
    VkDeviceSize miss_sz = align_up(hstride, balign);
    VkDeviceSize hit_sz  = align_up(hstride * ng, balign);
    VkDeviceSize sbt_size = rgen_sz + miss_sz + hit_sz;
    auto sbt = mk_buf(ctx, sbt_size,
                      VK_BUFFER_USAGE_SHADER_BINDING_TABLE_BIT_KHR |
                      VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                      VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                      VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, true);
    if (!sbt) return std::unexpected(sbt.error());
    {
        std::uint8_t* p = nullptr;
        vkMapMemory(dev, sbt->mem, 0, sbt_size, 0, (void**)&p);
        std::memset(p, 0, sbt_size);
        // raygen = group 0
        std::memcpy(p, handles.data() + hsize * 0, hsize);
        // miss = group 1
        std::memcpy(p + rgen_sz, handles.data() + hsize * 1, hsize);
        // hit = groups 2..2+ng, packed at hstride within the hit region
        for (std::uint32_t i = 0; i < ng; ++i)
            std::memcpy(p + rgen_sz + miss_sz + hstride * i,
                        handles.data() + hsize * (2 + i), hsize);
        vkUnmapMemory(dev, sbt->mem);
    }
    VkDeviceAddress base = buf_addr(ctx, sbt->buf);
    ...
    VkImage image = VK_NULL_HANDLE;
    if (vkCreateImage(dev, &ici, nullptr, &image) != VK_SUCCESS)
        return std::unexpected("rtx_trace_groups: create image failed");
    VkMemoryRequirements imr{};
    vkGetImageMemoryRequirements(dev, image, &imr);
    int it = find_mem(ctx.physical_device(), imr.memoryTypeBits,
                      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
