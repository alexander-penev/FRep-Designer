...
## [4.54.1] — figure scene: the default scene plus four twisted columns
- New `examples/07_default_twisted_columns.json` — the scene behind `docs/gallery/fig1_scene_only.png`, as a hand-editable file rather than C++ inside a tool. It reproduces `build_default_scene()` (gui/main.cpp) node for node — same ids, radii, translations and materials: the red Sphere(1), the blue Translate(-2.5)->Box(0.8), the green Translate(2.5)->Intersection(Box(0.9), Sphere(1.15)), the yellow SmoothUnion(k=0.4) blob pair, and the grey Plane(0,1,0,d=1.7) floor at y=-1.7 — then adds a back row of four Translate->TwistY->RotateY->Box(0.54, 2.45, 0.54) columns at tz=-5.32, ty=0.75, so their bases sit exactly on the floor.
- The four columns are identical in size and differ only in twist: tx/k/phase = -3.75/+0.96/0, -1.25/+0.80/1.025rad, +1.25/+1.25/0.131rad, +3.75/-1.05/1.527rad. Columns 1 and 4 are near-equal rates in opposite directions, 3 is the fastest and 2 the slowest. Since a square cross-section repeats every 90deg, the 87.5deg on column 4 is effectively zero — three of the four carry no real phase offset, and the differing "starting rotation" in the figure comes from the differing rates plus each column's own azimuth to the eye. The RotateY sits *under* the TwistY, so total rotation at height y is k*y + a: a pure phase offset that leaves the rate alone. Fitting tx freely gave -3.82/-1.25/+1.26/+3.80; the committed values snap that to a uniform 2.5 spacing (the front row's spacing, offset by 1.25), moving each column under 0.08 world units.
- Column geometry was solved back from the figure, not estimated by eye. A hand-rolled PNG reader plus a Nelder-Mead fit over the horizon row, the sphere's silhouette and the box's eight projected corners pins the camera used for the analysis to 1.18 px rms over 8 features. The column silhouettes plus the row where columns 2 and 3 meet the floor — both end at row 489, which is what breaks the depth/size degeneracy, since a farther and proportionally thicker column projects identically — give tz, hx and the height at 2.26 px rms over 14 features.
- k and a come from ray-probing rather than any closed form: cast one ray per image row down a candidate column's screen axis, record the rows where the surface normal jumps (a box edge crossing the axis), and search (k, a) for the pair reproducing the rows measured in the figure. Columns 1, 3 and 4 match their two measured crossing rows to 1 px.
- Three simpler estimators were tried first and each failed differently, which is why the probe exists: (1) autocorrelating the shading pattern locked onto the second harmonic — plotting the whole correlation curve exposed it, since a genuine period shows a negative dip at half the lag (column 1 does) while column 2's curve is high at every lag from 0.4 to 3.7, the signature of a smooth gradient with no repeat at all; (2) a closed-form phase formula was wrong because the crossing is observed in the view frame while RotateY acts in the world frame, which differ by 5-15deg per column; (3) a silhouette-width fit is biased because a twisted box's silhouette is a ruled surface, not the per-height cross-section the model assumed — it wanted k=0.76 for a column the probe puts at 0.96.
- Honest limit: column 2's rate is the one number resting on visual comparison rather than measurement. Its side faces are lit too similarly for the probe to resolve edge crossings — exactly one shows up, at row 358, and only at the loosest detection threshold, so a crossing *count* is not trustworthy for it. k=0.80 is where side-by-side comparison of renders against the figure puts it (0.55 reads as too few turns, 1.10 as twice too many); the phase then follows from the single visible crossing. Column albedos are ratios recovered from the figure (shading preserves them) scaled to a 0.8 maximum; absolute albedo magnitude is not recoverable from a render.
- Camera: the committed file carries position (0, 1.5, 8.82), target origin, fov 55 — an orbit camera, i.e. one the GUI viewport can express. This is *not* the camera fitted to the figure, which lands at (0, 1.13, 8.33) looking at (0, -0.30, 0) with fov 56.3. That fit's tilt is an artefact: fig1_scene_only.png is a viewport screen capture, and re-fitting with the target pinned to the origin plus a vertical crop offset reaches the same 1.09 px rms, so the crop is what moves the optical centre. The figure's exact 807x903 framing therefore isn't reproducible from an orbit camera; the column geometry is unaffected, being solved in world space.
- Camera caveat found while matching the figure: loading a scene in the GUI does NOT restore its camera. Viewport::recompute_camera() (gui/viewport.cpp) unconditionally overwrites scene_->camera().position/target/up from the viewport's own orbit state, which starts at the hardcoded cam_yaw_=0, cam_pitch_=0.3, cam_dist_=9 (gui/viewport.hpp) — the file's camera never reaches it. The real-time Vulkan viewport seeds its orbit from the scene (orbit_init_); the classic viewport does not. Headless rendering reads the file's camera correctly.
- Fixed a phantom tool name in the docs and in CI: `frep_gpu_render` does not exist and has no target in CMakeLists.txt, yet README.md, examples/README.md and .github/workflows/ci.yml all invoked it — the CI "Render every example scene" step could never have passed. The tool that takes a scene file is `frep_multipath <scene.json> [--width N --height N --out FILE.ppm]`; `frep_render` and `frep_advanced` build their demo scenes in C++ and accept no `--scene`, and `frep_designer --scene FILE` opens one in the GUI. All three call sites now use frep_multipath. Not executed here (no build tree), so the change is verified against the tool's argument parser, not by running CI.
- Verified against the deserializer (core/io/scene_io.cpp): every `type` string, parameter name and child arity in the file is one the loader accepts (TwistY/RotateY 1 child, Intersection/SmoothUnion 2), and all nine object ids are unique. Geometry was checked by an independent raymarcher mirroring each node's eval() — including TwistY's Lipschitz correction and the cubic smin — since no build tree is present here; the real render paths were not exercised.
- examples/README.md documents the scene, its parameter table, how each number was obtained and the camera caveat.

## [4.54.0] — adaptive SIMD width + domain-safe imported-expression math
- SIMD SDF evaluation now auto-selects the float-lane width from the host CPU: AVX-512F -> 16, AVX2 -> 8, else 4 (native_simd_width() in compile_sdf.hpp, via llvm::sys::getHostCPUFeatures). compile_scene_sdf_simd(scene) with no explicit width uses it; pass a width to override for A/B tests. Measured on an AVX-512 host: width-16 vs width-8 is ~1.12-1.37x faster across s1-s5 (sphere 1.13x, csg 1.21x, blend 1.12x, gyroid 1.37x, twist 1.26x) — the gain lands on exactly the polynomial/CSG scenes where libfive's array evaluator was ahead. Trig-heavy micro-cases don't gain in isolation (vector sin/cos don't widen), but full scenes carry enough surrounding arithmetic to still benefit.
- Domain-safe transcendentals in CustomExpr (imported analytic scenes). Raw llvm.pow returns NaN for a negative base with a fractional exponent, and raw sqrt/acos return NaN outside their domain — which turned whole converted scenes (the MPR/libfive involute-gear scenes) into 100% NaN and a black screen. Now: pow(a,b) = copysign(|a|^b, a) (odd-nth-root behaviour for negative bases; identical to plain pow for a>=0, so the blend h^k case is unchanged), sqrt clamps its argument at 0, and asin/acos clamp to [-1,1]. Verified: the gear scenes go from 100% NaN to 0% NaN; the smooth-blend scene (which uses pow) still has a proper inside region and no NaN.
- Note: removing the NaN is necessary but not sufficient for the gear scenes — they still lack a clean inside region because the MPR->FRep converter flattens libfive's distinct OP_POW and OP_NTH_ROOT operators into one pow(). The converter fix (emitting nth_root where the source used it) is tracked separately.
- Benchmark: frep4_bench/bench_grid now uses the adaptive width and reports the actual width in the backend name (cpu_ir-simd8 / cpu_ir-simd16).
