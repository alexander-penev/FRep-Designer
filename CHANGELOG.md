...
- The columns are identical in size and differ only in twist: tx/k/phase = -3.75/+1.00/76.5deg, -1.25/+1.10/13.2deg, +1.25/+1.21/36.3deg, +3.75/-1.00/9.1deg. The fitted tx were -3.82/-1.25/+1.26/+3.80 and are snapped to a uniform 2.5 spacing (the front row's spacing, offset by 1.25) — each moves by under 0.08 world units, inside the fit's residual.
- Rates and phases come from the rows where a face edge crosses a column's screen axis — a corner pointing at the camera, i.e. `a = beta - 45deg - k*y` with beta the column-to-eye azimuth. Columns 1 and 3 have two visible crossings each and validate themselves: the two independent rows agree on the phase to 0.2deg, and the crossing spacing gives k directly (1.574 world units per quarter turn on column 1 -> k = 0.998, matching the assumed 1.00). Two earlier estimators were discarded after this: autocorrelation of the shading pattern locked onto the second harmonic on column 2 (reporting k = 0.55 for a true ~1.10), and the silhouette-width fit is systematically biased because a twisted box's silhouette is a ruled surface, not the per-height cross-section my model assumed (it wanted k = 0.76 for a column measured at 0.998). The beta term matters: the crossing condition is observed in the view frame while RotateY acts in the world frame, and the two differ by 5-15deg per column. Columns 1 and 4 are the same rate in opposite directions, 2 is the slow one, 3 the fastest. The RotateY sits *under* the TwistY so the total rotation at height y is k*y + a — a pure phase offset that leaves the rate alone.
