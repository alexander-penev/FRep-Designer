...
- The columns are identical in size and differ only in twist: tx/k/phase = -3.75/+0.96/0deg, -1.25/+0.54/52.5deg, +1.25/+1.25/7.5deg, +3.75/-1.05/87.5deg. Since a square cross-section repeats every 90deg, 87.5deg is effectively zero — three of the four columns carry no real phase offset, and the differing "starting rotation" seen in the figure comes from the differing rates plus each column's own azimuth to the eye. The fitted tx were -3.82/-1.25/+1.26/+3.80 and are snapped to a uniform 2.5 spacing (the front row's spacing, offset by 1.25) — each moves by under 0.08 world units, inside the fit's residual.
- k and a were found by ray-probing rather than any closed form: cast one ray per image row down a candidate column's screen axis, record the rows where the surface normal jumps (a box edge crossing the axis), and search (k, a) for the pair that reproduces the rows measured in the figure. Columns 1, 3 and 4 match their two measured crossing rows to 1 px. Column 2 is pinned by a count instead of a position — over rows 168-486 the figure has exactly one crossing, which k=1.10 turns into 3-4 and k=0.80 into 2-3, leaving k in the 0.42-0.55 band.
- Three simpler estimators were tried first and each failed in its own way, which is why the probe exists: (1) autocorrelating the shading pattern locked onto the second harmonic — plotting the whole correlation curve exposed it, since a genuine period shows a negative dip at half the lag (column 1 does) while column 2's curve is high at every lag from 0.4 to 3.7, the signature of a smooth gradient with no repeat at all; (2) a closed-form phase formula was wrong because the crossing is observed in the view frame while RotateY acts in the world frame; (3) a silhouette-width fit is biased because a twisted box's silhouette is a ruled surface, not the per-height cross-section the model assumed — it wanted k=0.76 for a column the probe puts at 0.96. Columns 1 and 4 are the same rate in opposite directions, 2 is the slow one, 3 the fastest. The RotateY sits *under* the TwistY so the total rotation at height y is k*y + a — a pure phase offset that leaves the rate alone.
- Camera and column geometry were solved back from the reference image, not estimated by eye. A hand-rolled PNG reader plus a Nelder-Mead fit over the horizon row, the red sphere's silhouette and the blue box's eight projected corners gives eye (0, 1.13, 8.33), target (0, -0.30, 0), fov 56.3 at 1.18 px rms over 8 features. The column silhouettes (x extents and top rows) plus the row where columns 2 and 3 meet the floor — both end at row 489, which is what breaks the depth/size degeneracy, since a farther and thicker column projects identically — then give tz, hx and the height at 2.26 px rms over 14 features. Twist rates come from autocorrelating each column's shading pattern along its own axis in world-y; column 1 is corroborated by an independent measurement of its face-edge spacing (k = 0.999 vs 0.935).
- Honest limits: phases are the weakest number (the width-profile fit they come from has 13-28 px rms), and the sign of k per column rests on reading the drift direction of each column's internal shading boundary — the magnitudes are solid, the directions less so. Column albedos are ratios recovered from the image (shading preserves them) scaled to a 0.8 maximum; the absolute albedo magnitude is not recoverable from a render.
...
- examples/README.md lists the scene, its parameter table and how the numbers were obtained.
